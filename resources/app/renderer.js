  ...
  torrentClient.add(torrentId, function (torrent) {
    ...
    torrent.files.forEach(function (file, index) {
      if(file.name.endsWith('.pkg'))
      {
        if (index == 1)
        {
          alert("Fichier trouver:" + "\n" + "\n" + index + " - " + file.name);
        $("#listpkg").append('<li><spam class="PKG" id="pkg' + index + '">' + file.name + '</spam><input type="button" value="▼" id="pkg-' + index + '" onclick="Downlist(this.id)" /></li>');
        } 
        else
        {
          alert("Fichier trouver:" + "\n" + "\n" + index + " - " + file.name);
        $("#listpkg").append('<li><spam class="PKG" id="pkg' + index + '">' + file.name + '</spam><input type="button" value="▲" id="pkg-' + index + '" onclick="Uplist(this.id)" /><input type="button" value="▼" id="pkg-' + index + '" onclick="Downlist(this.id)" /></li>');
        }

        $("#ModalPKG").css("display", "block");
      }     
    });
...
jQuery('#Valider').on('click', function () {
  var listpkglenght = $("#listpkg").find("li").length;

  for (var i=1; i<listpkglenght + 1; i++) {
    var filename = $("#pkg" + i).text();
    queue.push({
        "type": "direct",
        "packages": [`http://${document.querySelector('#localIP').value}:${jQuery('#torrentPortNumber').val()}/${i}/${filename}`]
    });
  }
...
jQuery('#ValiderTheme').on('click', function () {
  if (document.getElementById("ThemeTXT").value == "")
  {
    alert("Veuiller saisir le nom du thème!");
  }
  else
  {
